...
const DropDownDepositor = ({content, Language} : {content : any, Language: String}) => {
  ...
  const parts = userName ? userName.split(' ', 2) : null;
  const part1 = parts ? parts[0].split("")[0].toUpperCase() : '';
  const part2 = parts ? parts[1].split("")[0].toUpperCase() : '';
  const LetterFullName = `${part1}${part2}`;
...
// Depositor Switch to French Language
  ...
  return (
    <DropdownMenu >
        <DropdownMenuTrigger asChild className='focus-visible:ring-neutralBg focus:ring-neutralBg focus:border-neutralBg focus-visible:border-neutralBg focus:outline-neutralBg focus-visible:ring-offset-0'>
        <Button  className="border-0 flex bg-neutralBg items-center gap-2 hover:bg-neutralBg hover:text-primary focus-visible:ring-neutralBg focus:ring-neutralBg focus-visible:ring-offset-0">
            ...
            <div className="flex flex-col justify-start items-start">{ status !== "loading" && userName ?
                <>
                    <h4 className="text-secondaryDarkBlue">{userName}</h4>
                    <p className="text-xs text-primary">{userTypeSwitch} {content('Account')}</p>
                </>
                    :
                <>
            ...
            { status !== "loading" && userName  ? <Image src={'/icons/arrow-down.svg'} width={18} height={18} alt="ArrowDown"/> : 
